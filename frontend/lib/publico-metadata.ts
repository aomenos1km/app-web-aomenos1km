...
function toAbsoluteUrl(url: string) {
  if (!url) return `${SITE_BASE}/logo-aomenos1km.png`
...
export function buildEventoMetadata(evento: EventoPublicoShare | null, path: string): Metadata {
  ...
  if (!evento) {
    return {
      ...
      openGraph: {
        ...
        images: [
          {
            url: toAbsoluteUrl('/logo-aomenos1km.png'),
            width: 1200,
            height: 630,
            alt: 'Aomenos1km',
          },
      ...
      twitter: {
        ...
        title: `Check-in do Evento | ${TITLE_SITE_DEFAULT}`,
        description: 'Faça seu check-in no evento da Aomenos1km.',
        images: [toAbsoluteUrl('/logo-aomenos1km.png')],
      },
    }
  }
  ...
  const imagem = toAbsoluteUrl(evento.capa_url || '/logo-aomenos1km.png')
